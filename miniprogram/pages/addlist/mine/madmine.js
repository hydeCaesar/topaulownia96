// miniprogram/pages/addlist/mine/madmine.js
var app = getApp()
Page({
  data: {
  ...
  setgamearr: function(row, column, bomb) { //根据行列设置游戏二维数组（地图）
    ...
    for (var i = row - 1; i >= 0; i--) {
      ...
      } //val用来记录周边雷的数量，cover用来记录是否翻开：无dom操作只能用数据记录状态
    }
    console.log(arrmap)
    var arr = []; //一维自然数
    for (var k = row * column - 1; k >= 0; k--) {
      arr[k] = k
    }
    //随机炸弹位置
    for (var h = bomb - 1; h >= 0; h--) {
      var seat = arr.splice(Math.floor(Math.random() * arr.length), 1)[0]
      var r = Math.floor(seat / column),
        c = Math.floor(seat % column);
      //console.log(seat+'\n'+r+","+c);
      arrmap[r][c].val = "B";
  ...
  addcount: function(r, c, arrmap) {
    var that = this;
    if (r - 1 >= 0) { //九宫格上三个
      if (c - 1 >= 0 && arrmap[r - 1][c - 1].val != "B") {
        arrmap[r - 1][c - 1].val++
      }
      if (arrmap[r - 1][c].val != "B") {
        arrmap[r - 1][c].val++
      }
      if (c + 1 < app.column && arrmap[r - 1][c + 1].val != "B") {
        arrmap[r - 1][c + 1].val++
      }
    }
    ...
    //九宫格左右两个
    if (c - 1 >= 0 && arrmap[r][c - 1].val != "B") {
      arrmap[r][c - 1].val++
    }
    if (c + 1 < app.column && arrmap[r][c + 1].val != "B") {
      arrmap[r][c + 1].val++
    }
    return arrmap;
  },
  ...
  taphandler: function(e) {
    ...
    var r = e.currentTarget.dataset.row,
      c = e.currentTarget.dataset.column; //无法想jquery获得index，只能用数据记录，直接获取喽
